...
class User(db.Model):
    ...
    # NOTE: We DON'T store last_offset here - User must manage it themselves
...
class Message(db.Model):
    id = db.Column(db.Integer, primary_key=True)  # This is our update_id
    message_id = db.Column(db.String(64), unique=True, nullable=False)
    message_type = db.Column(db.String(20), nullable=False)  # text, file, reply
    ...
    def to_dict(self):
        message_data = {
            'update_id': self.id,  # Important: This is the offset key
...
def get_message_preview(message):
    """Get text preview for reply messages"""
...
def cleanup_old_messages():
    """Delete messages older than MESSAGE_AUTO_DELETE_HOURS"""
    try:
        cutoff_time = datetime.utcnow() - timedelta(hours=MESSAGE_AUTO_DELETE_HOURS)
...
def send_message():
    ...
    try:
        ...
        expires_at = datetime.utcnow() + timedelta(hours=MESSAGE_AUTO_DELETE_HOURS)
        ...
        return jsonify({
            ...
            'data': {
                'message_id': message_id,
                'update_id': message.id,  # Return the update_id for client to save
...
# Telegram-style getUpdates with EXACT Telegram logic
@app.route('/api/getUpdates', methods=['GET'])
@require_api_key
def get_updates():
    """
    EXACT Telegram-style getUpdates implementation:
    
    - WITHOUT offset: Returns ALL pending updates (including old ones)
    - WITH offset: Returns only updates with update_id > offset
    - Client MUST save and send offset to avoid duplicates
    - Server does NOT store client's offset
    """
    try:
        user = request.user
        ...
        # Telegram Logic: If no offset, return ALL pending updates
        if offset is None:
            # Get ALL messages for this user (this will include old ones)
            query = Message.query.filter(Message.receiver_id == user.user_id)
        else:
            # Get only messages with update_id > offset
            query = Message.query.filter(
                Message.receiver_id == user.user_id,
                Message.id > offset
            )
        
        # Long polling if timeout > 0
        if timeout > 0:
        ...
        for message in messages:
            ...
            # Mark as delivered when retrieved
        ...
        # Calculate next_offset (max_update_id + 1)
...
# Home route with detailed instructions
@app.route('/')
def home():
    return jsonify({
        'service': 'EXACT Telegram-style Messaging API',
        'version': '4.0.0',
        'description': 'Implements EXACT Telegram getUpdates logic with offset handling',
        'important_notes': [
            ...
            '📝 Client is responsible for managing offset (like Telegram bots)'
        ],
        'usage_examples': {
            'first_call': 'GET /api/getUpdates (returns all pending messages)',
            'next_calls': 'GET /api/getUpdates?offset=<next_offset> (returns only new messages)',
            'send_text': 'POST /api/sendMessage',
            'send_file': 'POST /api/sendFile'
        },
        'endpoints': {
            'auth': {
            ...
            'messages': {
                ...
                'getUpdates': 'GET /api/getUpdates',
                'getChatHistory': 'GET /api/getChatHistory'
            },
            'files': {
                'download': 'GET /api/files/download/<file_id>'
            },
            'webhook': {
                'setWebhook': 'POST /api/setWebhook'
            }
        }
    })
...
if __name__ == '__main__':
    init_db()
    print("EXACT Telegram-style API starting on http://localhost:3000")
    print("\n=== IMPORTANT ===")
    ...
    print("Client MUST manage offset to avoid duplicates")
    print("=================\n")
