...
from datetime import datetime, timedelta, timezone  # ✅ timezone aware datetime
import secrets
import os
import sqlite3
...
class User(db.Model):
    ...
    last_activity = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # ✅ Fixed
...
class Message(db.Model):
    ...
    sent_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # ✅ Fixed
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        user.last_activity = datetime.now(timezone.utc)  # ✅ Fixed
        db.session.commit()
...
def send_webhook_notification(user_id, message_data):
    try:
        ...
        webhook.last_triggered = datetime.now(timezone.utc)  # ✅ Fixed
...
def cleanup_old_messages():
    try:
        cutoff_time = datetime.now(timezone.utc) - timedelta(hours=MESSAGE_AUTO_DELETE_HOURS)  # ✅ Fixed
...
def send_message():
    try:
        ...
        expires_at = datetime.now(timezone.utc) + timedelta(hours=MESSAGE_AUTO_DELETE_HOURS)  # ✅ Fixed
...
if __name__ == '__main__':
    ...
    # ✅ Render compatible - use 0.0.0.0 and environment port
